...
function angleBetweenClockHands(date) {
  ...
  const hh = utcDate.getUTCHours() > 12 ? utcDate.getUTCHours() - 12 : utcDate.getUTCHours();
  const mm = utcDate.getUTCMinutes();
  const angleH = 0.5 * (60 * hh + mm);
  const angleM = 6 * mm;
  const dif = angleH - angleM > 180 ? angleH - angleM - 180 : angleH - angleM;
